# Banner baker delegates chart decoding to `decode:chart`

ADR 0004 built the banner baker as a self-contained pipeline: it decoded the
...
source). This ADR **removes the baker's decoder and has `bake:header` shell out
to `decode:chart`** for the chart→bitmap step.

**The baker keeps only its banner-specific tail.** `bake:header` still owns the
work that is about the *banner*, not the chart: trimming the sprite to its
bounding box, the bake-time mirror (ADR 0006), the fixed-5-row aspect scale
(ADR 0007/0008), the chafa quadrant fold, and emitting `banner.ts`. The header's
...
white-coded cells now survive into the banner, and cell colours are the legend's
...
`banner.ts` is committed alongside this ADR so the artifact matches the pipeline.

**bake:header now takes an arbitrary source and forwards `--scale`/`--dedither`.**
...
`--scale` controls the resolution of the intermediate sprite bitmap; the banner
still resamples down to its fixed 5 rows, so scale is a knob on the decode
stage's fidelity, not on the banner's size.
...
the stronger one; the baker is ~230 lines lighter and purely about banner
geometry. `bake:header` gains an arbitrary source argument and `--scale` /
...
with this ADR so `banner.ts` tracks the pipeline that generates it.
